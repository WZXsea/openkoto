...
const DISABLED_MESSAGE: &str =
    "External tools are disabled. Set OPENKOTO_ENABLE_EXTERNAL_TOOLS=1 or unset OPENKOTO_ENABLE_EXTERNAL_TOOLS to enable this command for the current session.";
...
mod tests {
    ...
    #[test]
    fn external_tools_are_enabled_by_default() {
    ...
    #[test]
    fn external_tools_can_be_disabled_by_env() {
        let _guard = ENV_LOCK.lock().unwrap();
        let previous = std::env::var_os(EXTERNAL_TOOLS_ENV);
        std::env::set_var(EXTERNAL_TOOLS_ENV, "0");
